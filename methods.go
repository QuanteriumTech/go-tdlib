...
// GetCountryCode Uses current user IP address to found their country. Returns two-letter ISO 3166-1 alpha-2 country code. Can be called before authorization
...
func (client *Client) TestUseUpdate() (Update, error) {
	...
	case UpdateChatSourceType:
		var update UpdateChatSource
		err = json.Unmarshal(result.Raw, &update)
		return &update, err
